...
public class Game : MonoBehaviour
{
    ...
    public GameObject GetAttackPiece()
    {
        return attackSim;
    }    
    ...
    public void MateSimulation()
    {
        ...
        if (checkmate == true && check == true)
            Checkmate();
        else if (checkmate == true && check == false)
            Stalemate();
    ...
    public void Checkmate()
    {
        if (checkmate == true && check == true)
        {
            ...
            if (currentPlayer == "black") Winner("white");
        }
    }
    ...
    public void Synchronization()
    {
        for (int x = 0; x < 8; x++)
            for (int y = 0; y < 8; y++)
                if (positions[x, y] != simulation[x, y])
                    simulation[x, y] = positions[x, y];
    }
